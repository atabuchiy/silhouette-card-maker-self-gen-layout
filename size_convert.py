...
def size_to_pt(size_string):
    ...
    if mm_match:
        size_mm = float(mm_match.group(1))
        return math.floor(size_mm / 25.4 * 72)

    # Match inches
    in_match = re.fullmatch(rf"({float_pattern})in", size_string)
    if in_match:
        size_in = float(in_match.group(1))
        return math.floor(size_in * 72)
...
def size_to_mm(size_string):
    ...
    if mm_match:
        size_mm = float(mm_match.group(1))
        return math.floor(size_mm)

    # Match inches
    in_match = re.fullmatch(rf"({float_pattern})in", size_string)
    if in_match:
        size_in = float(in_match.group(1))
        return math.floor(size_in * 25.4)
...
def size_to_in(size_string):
    ...
    if mm_match:
        size_mm = float(mm_match.group(1))
        return math.floor(size_mm / 25.4)

    # Match inches
    in_match = re.fullmatch(rf"({float_pattern})in", size_string)
    if in_match:
        size_in = float(in_match.group(1))
        return math.floor(size_in)
